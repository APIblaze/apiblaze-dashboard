...
export async function GET(
  request: NextRequest,
  context: { params: Promise<{ projectId: string; apiVersion: string; pathEncoded: string; method: string }> }
) {
  try {
    const { projectId, apiVersion, pathEncoded, method } = await context.params;
    ...
    const data = await client.request<Record<string, unknown>>(
      `/route-configs/${encodeURIComponent(projectId)}/${encodeURIComponent(apiVersion)}/${pathEncoded}/${encodeURIComponent(method)}`,
...
export async function PUT(
  ...
  try {
    ...
    return NextResponse.json(
      { error: 'Failed to save route config', details: message },
      { status: 500 }
    );
  }
}
